...
/** modified to suit my needs (added more functions), original sources can be found
 * at https://math.hws.edu/eck/cs124/javanotes6/source/TextIO.java
 * and https://math.hws.edu/eck/cs124/javanotes3/c10/ex-10-1-answer.html
 * */

public class FileProcessing {

        public final static char EOF = (char)0xFFFF; // identify end-of-file
    ...
    public static ArrayList<WordData> words = new ArrayList<WordData>();   // to hold the words from the file.

    public static int wordCount;   // The number of words currently stored in the array.

    public static void insertWord(String w) {
        ...
        int pos = 0;  // This will be the position in the array list where w belongs.
        ...
        w = w.toLowerCase();
         ...
         /* Find the position in the array where w belongs, after all the
            ...
            and return. */
        if (pos == 0)
            words.add(pos, new WordData(w));
        else
            while (pos < wordCount && words.get(pos).word.compareTo(w) < 0)
                pos++;

        if (pos < wordCount && words.get(pos).word.equals(w)) {
            words.get(pos).count++;
            return;
        }
        ...
        for (int i = wordCount; i > pos; i--)
            ...
            words.set(pos, new WordData(w));
            wordCount++;

    }  // end insertWord()


        // Read from standard input. CLoses the previous input source.
